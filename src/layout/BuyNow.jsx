...
<div className={`${fullname!=null?"flex":"hidden"} h-20  items-center cursor-pointer justify-center shadow-xl rounded-md`} onClick={
  handleOpen
} >
    <AddIcon/>
...
<div className=' flex p-4 mt-10 flex-col  shadow-xl rounded-md'>
  ...
  <p className='lg:px-10'>Qty:{state.state.count}</p>
    ...
    <div className='flex flex-col '>
        <p className='text-sm text-gray-500'>{state.state.count} Item(s). Subtotal:<span className='text-green-600'>${state.state.price}</span></p>
       
    </div>
    </div>
</div>
        </div>
<div className='flex p-4 w-[400px] flex-col   shadow-xl rounded-md'>
...
<div className='flex justify-between '>
  ...
  <p className='text-sm'>${state.state.price}</p>
</div>
<div className='flex justify-between '>
...
<div className='flex justify-between '>
  ...
  <p className='text-sm'>${state.state.price+charge}</p>
</div>
...
<button className='h-10 w-80 mt-5 ml-4 bg-green-800  text-white' onClick={()=>{
    ...
    placeorder({fullname,phonenumber,address,landmark,province,city,area,products:[{
    ...
    }],totalprice:state.state.price+charge,navigate})
  }}>
      Place Order
  </button>
</div>
